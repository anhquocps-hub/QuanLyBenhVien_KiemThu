...
try {
    ...
    if (initialRows == 0) {
        KeywordUtil.markFailedAndStop('Room table has no data before search; cannot validate positive search flow.')
    }
    ...
    if (!probe) {
        KeywordUtil.markFailedAndStop('Could not extract room search probe from first row.')
    }
