...
try {
    ...
    if (initialRows == 0) {
        KeywordUtil.markFailedAndStop('Invoice table has no data before search; cannot validate positive search flow.')
    }
    ...
    if (!probe) {
        KeywordUtil.markFailedAndStop('Could not extract invoice search probe from first row.')
    }
